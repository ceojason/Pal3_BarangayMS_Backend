...
public class AdminEnrollmentServiceImpl extends BaseServiceImpl implements AdminEnrollmentService {
    ...
    public SystemAdminModel validate(SystemAdminModel modelObj) {
        ...
        if (modelObj.getFirstNm()==null){
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.FIRST_NM));
        }else{
        ...
        if (modelObj.getLastNm()==null){
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.LAST_NM));
        }else{
            modelObj.setLastNm(modelObj.getLastNm().trim().toUpperCase());
        }

        if (modelObj.getGender()==null) {
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.GENDER));
        }else{
            modelObj.setGenderDscp(GenderEnum.getGenderDscpFromKeyStr(modelObj.getGender()));
        }

        if (modelObj.getHomeAddress()==null) {
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.HOME_ADDRESS));
        }else{
            modelObj.setHomeAddress(modelObj.getHomeAddress().trim().toUpperCase());
        }

        if (modelObj.getMobileNo()==null) {
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.MOBILE_NO));
        }else{
        ...
        if (modelObj.getEmailAddress()==null) {
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.EMAIL));
        }else{
            modelObj.setEmailAddress(modelObj.getEmailAddress().trim());
        }

        if (modelObj.getBday()==null) {
            errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.IS_REQUIRED_SUFFIX, StringMessagesUtils.BIRTHDAY));
        }else{
            ...
            if (isValidDt) {
                ...
                errorList.add(StringMessagesUtils.formatMsgString(StringMessagesUtils.INVALID_OBJ, "Birth Date"));
            }
        }
